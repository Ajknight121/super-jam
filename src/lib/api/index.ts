export async function handleJsonResponse<T>(res: Response, parseFn?: (json: any) => T): Promise<T | undefined> {
    if (!res.ok) {
    ...
    try {
        ...
        throw new Error (`Response had unsupported content-type: ${contentType} (not JSON).`);
    }
}